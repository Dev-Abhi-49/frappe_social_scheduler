...
class InstagramProvider(BaseProvider):
    ...
    def _publish_feed_post(
        ...
        """
        Publish Instagram Feed Post
        Supports: single image, single video (as reel), carousel (2-10 images/videos)
        Uses container creation -> publish workflow
        """
        try:
            if not media_files or len(media_files) == 0:
    ...
    def _publish_single_media(
        ...
        """
        Publish single image or video to Instagram feed
        Note: As of 2024, Instagram requires all single videos to be published as REELS
        """
        try:
            file_url = getattr(media_file, "file_url", None) or media_file
            public_url = self._get_public_url(file_url)
            ...
            if is_video:
                ...
                # Pass is_feed_video flag to relax Reel validations
            ...
            # Publish container (images don't require processing wait)
            publish_resp = requests.post(
                f"{self.api_base}/{instagram_id}/media_publish",
                data={"creation_id": container_id, "access_token": page_token},
                timeout=60,
    ...
    def _publish_carousel(
        ...
        """
        Publish carousel post (2-10 images/videos)
        Note: Carousel videos can still use VIDEO media_type when marked as carousel items
        """
        try:
            item_container_ids = []
            ...
            for media_file in media_files:
                ...
                item_data = {
                    "access_token": page_token,
                    "is_carousel_item": "true",  # Critical for videos in carousel
                }

                if is_video:
                    # Carousel items can still use VIDEO when is_carousel_item=true
                    item_data["media_type"] = "VIDEO"
                    item_data["video_url"] = public_url
                else:
                    item_data["image_url"] = public_url
    ...
    def _publish_reel(
        ...
        Handles both explicit reels and auto-converted feed videos
        """
        if not media_files or len(media_files) != 1:
            return PublishResult(success=False, error_message="Reels require exactly one video")
        ...
        try:
            ...
            # Check if this is a feed video (relaxed validation) or explicit reel (strict validation)
            is_feed_video = kwargs.get("is_feed_video", False)

            # Validate duration (only for explicit reels)
            try:
                duration = self._get_video_duration(full_path)
                
                if not is_feed_video:
                    # Strict validation for explicit reels
                    if duration < self.REEL_MIN_DURATION or duration > self.REEL_MAX_DURATION:
                        return PublishResult(
                            success=False,
                            error_message=(
                    ...
                    # Relaxed validation for feed videos
            ...
            # Validate aspect ratio (warn only, don't fail)
            try:
                width, height = self._get_video_dimensions(full_path)
                if height <= width:
                    if not is_feed_video:
                        # Strict validation for explicit reels
                        frappe.logger().warning(
                            f"Reel video is not vertical (got {width}x{height}). "
                            "Vertical 9:16 aspect ratio is strongly recommended for Reels."
                        )
                    else:
                        # Just log for feed videos
                        frappe.logger().info(
                            f"Feed video dimensions: {width}x{height}. "
                            "Vertical format (9:16) is recommended for best results."
                        )
            except Exception as e:
            ...
            # Optional: Add cover/thumbnail offset (in milliseconds)
            thumb_offset = kwargs.get("thumb_offset")
            if thumb_offset is not None:
